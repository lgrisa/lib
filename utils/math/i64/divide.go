...
// 寻找最优使用方案
func GetDividePlan(totalAmount int64, amountArray, ownCountArray []int64) (ok bool, plan []int64) {
	...
	for i := 0; i < n; i++ {
		...
		if backCount > 0 {
			plan[i] = plan[i] - backCount
			overflow = overflow - amt*backCount
		}
	}
