...
import (
	...
	file2 "github.com/lgrisa/lib/utils/file"
	"github.com/lgrisa/lib/utils/log"
	"github.com/lgrisa/lib/utils/message"
...
func (i *initCiStruct) packAndSendFile() error {
	...
	for _, file := range i.activeDir {
		...
		//先压缩文件夹
		if file.IsDir() {
			fileName = fmt.Sprintf("%s.zip", file2.GetFilePrefix(file.Name()))
		...
		pngPath := fmt.Sprintf("%s/%s", QRCodePath, fmt.Sprintf("%s.png", file2.GetFilePrefix(newAppName)))
...
func (i *initCiStruct) sendDiscordMessage() error {
	...
	if err != nil {
		return errors.Errorf("init webhook fail failed,url:%v err: %v", DiscordUrl, err)
	}
	...
	for _, v := range i.sendMessageMap {
		...
		if i.tag != "" {
			fieldMap["执行Tag"] = i.tag
		}

		if i.commitText != "" {
			fieldMap["更新内容"] = i.commitText
		}

		fieldMap["二维码地址"] = v.PngUrl

		if err = message.SendDiscordNoGoroutines(&message.DiscordMessage{
			DiscordWebhookUrl:  DiscordUrl,
			DiscordRobotThread: RobotThread,

			AuthorName: "gitlab-runner自动打包完成",
			Title:      v.Title,
			Url:        v.Url,
			FieldMap:   fieldMap,

			DiscordFile: &message.DisCordFile{
				FilePath: v.PngPath,
				Title:    v.Title + ".png",
				Desc:     v.PngPath,
			},
			...
			return errors.Errorf("send discord message failed, err: %s", err)
		}
	}
