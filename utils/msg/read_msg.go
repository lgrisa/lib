...
func ReadMsg(data []byte) (int, int, []byte, error) {
	...
	return moduleId, msgId, newData, nil
}
...
func WriteMsg(modID, msgID int, m proto.Message) ([]byte, error) {
	data, err := proto.Marshal(m)
	if err != nil {
		return nil, errors.Wrap(err, "proto.Marshal")
	}

	b := protowire.AppendVarint(nil, uint64(modID))
	b = protowire.AppendVarint(b, uint64(msgID))
	b = append(b, data...)
	return b, nil
}
